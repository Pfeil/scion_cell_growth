...
def sobelThresh(img2):
    img2 = cv.GaussianBlur(img2, (25,25), 20)

    #img2_x = cv.Sobel(img2,cv.CV_64F,1,0,ksize=5)
    #img2_y = cv.Sobel(img2,cv.CV_64F,0,1,ksize=5)
    #img2_xy = np.abs(img2_x) + np.abs(img2_y)
    img2_xy = cv.Canny(img2, 1, 6)

    img2_xy = np.array(img2_xy)
    img2_xy = invert(img2_xy)

    #width, height = img2_xy.shape
    #thresh = 184
    #ret,img2_xy = cv.threshold(img2_xy, thresh, 255,  cv.THRESH_BINARY)

    kernel = np.ones((3,3),np.uint8)
    img2_xy = cv.erode(img2_xy,kernel,iterations = 3)
    #img2_xy = cv.morphologyEx(img2_xy, cv.MORPH_OPEN, kernel, iterations = 1)
    #cv.namedWindow('image', cv.WINDOW_NORMAL)
    #cv.imshow('image',img2_xy)
    #cv.waitKey(0)
    return img2_xy
